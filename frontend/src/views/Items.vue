<template>
  ...
  <v-main>
    <v-container class="py-6" ref="scrollContainer" @scroll.passive="handleScroll">
      ...
      >
        <v-tab v-for="(label, status) in statusMap" :key="status" :value="status">
          {{ label }}
        </v-tab>
      </v-tabs>
          ...
          <v-card class="rounded-xl d-flex justify-space-between align-center" elevation="2"
            ...
            <div class="flex-grow-1 d-flex flex-column justify-space-between pa-4"
              ...
              <div v-if="item.status === 'available'" class="d-flex flex-wrap gap-2 mt-2"
                ...
                <v-btn variant="tonal" color="purple" size="small" class="flex-grow-1" style="min-width: 110px"
                  @click.stop="openGiveDialog(item.code)">
                  <v-icon start>mdi-hand-heart</v-icon> 赠送出库
                </v-btn>
                <v-btn variant="tonal" color="error" size="small" class="flex-grow-1" style="min-width: 110px"
                  @click.stop="openLostDialog(item.code)">
                  <v-icon start>mdi-alert-circle</v-icon> 标记丢失
                </v-btn>
                <v-btn variant="tonal" color="primary" size="small" class="flex-grow-1" style="min-width: 110px"
                  @click.stop="openConfirmUseDialog(item.code)">
                  <v-icon start>mdi-check-circle</v-icon> 标记使用
                </v-btn>
                <v-btn variant="tonal" color="teal" size="small" class="flex-grow-1" style="min-width: 110px"
                  @click.stop="openMoveDialog(item.code)">
      ...
      <v-dialog v-model="dialog" max-width="500px">
        <v-card v-if="selected">
          ...
          <v-card-text>
            <v-row dense>
              <v-col cols="12" sm="6">
                ...
                <div class="text-body-1 mb-2"><strong>位置:</strong> {{ locationMap[selected.location_id] || '无' }}</div>
                <div class="text-body-1 mb-2"><strong>录入时间:</strong> {{ selected.received_at || 'N/A' }}</div>
              </v-col>
              <v-col cols="12" sm="6">
            ...
            <div v-if="selected.status === 'given' && selected.given_info">
              ...
              <div class="text-body-2 mb-1"><strong>被赠送人:</strong> {{ selected.given_info.to || '未记录' }}</div>
              <div class="text-body-2 mb-1"><strong>赠送方式:</strong> {{ selected.given_info.method || '未记录' }}</div>
              <div class="text-body-2 mb-1"><strong>赠送类型:</strong> {{ selected.given_info.giftType || '未记录' }}</div>
              <div class="text-body-2 mb-1"><strong>赠送活动:</strong> {{ selected.given_info.event || '无' }}</div>
              <div class="text-body-2"><strong>赠送时间:</strong> {{ selected.given_info.date || selected.last_action_date }}</div>
            </div>
          ...
          <v-card-actions>
            ...
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="openImageUploaderDialog(selected.code)">
              <v-icon start>mdi-image-edit</v-icon> 修改图片
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      ...
      <ImageUploaderDialog
        v-model:open="imageUploaderDialogOpen"
        :code="selectedCode"
        @submitted="onImageUploaded"
      ...
      <v-fab-transition>
        <v-btn v-if="showBackToTop" icon large class="back-to-top" color="primary" @click="scrollToTop">
...
import ImageUploaderDialog from '@/components/ImageUploaderDialog.vue' // <-- 新增
...
const imageUploaderDialogOpen = ref(false) // <-- 新增
...
function openImageUploaderDialog(code) { // <-- 新增
  selectedCode.value = code
  imageUploaderDialogOpen.value = true
}
...
function onImageUploaded(photoUrl) {
  // 更新当前选中的 item 的 photo_url
  if (selected.value) {
    selected.value.photo_url = photoUrl
  }
}
